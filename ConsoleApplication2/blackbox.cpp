...
void wyswietl_menu() {
	cout << "Wybierz plansze do gry:" << endl;
	cout << "5: Plansza 5x5 z trzema ukrytymi atomami." << endl;
	cout << "8: Plansza 8x8 z piecioma ukrytymi atomami." << endl;
	cout << "10: Plansza 10x10 z osmioma ukrytymi atomami." << endl;
}

void poziom_gry(Plansza& p) {
...
void dodaj_ruch(Stos& stos, int x, int y, char wynik) {
	Ruch ruch;
	ruch.x = x;
	ruch.y = y;
	ruch.wynik = wynik;
	stos.ruchy_gracza[stos.licznik] = ruch;
	stos.licznik++;
	stos.indeks_ostatniego_ruchu = stos.licznik;
}

void cofnij_ruch(Plansza& p, Stos& stos) {
	int indeks = stos.licznik - 1;
	if (indeks < 0) {
		return;
	}
	Ruch ostatni_ruch = stos.ruchy_gracza[indeks];
	int x = ostatni_ruch.x;
	int y = ostatni_ruch.y;
	if (ostatni_ruch.wynik == 'o') {
...
void powtorz_cofniety_ruch(Plansza& p, Stos& stos) {
	if (stos.licznik >= stos.indeks_ostatniego_ruchu)
	{
		return;
	}
	stos.licznik++;
...
void sterowanie(int& x_gracza, int& y_gracza, int x_max, int y_max, bool& czy_pokazac_atomy, bool& czy_przerwac_gre, bool& koniec_gry, bool& czy_poddajesz_sie, Plansza& p, Stos& stos) {
	char operacja;
	operacja = getchar();
	...
	case 'r':
		...
		break;
	case 'k':
		int liczba_poprawnie_zaznaczonych_atomow = 0;

		for (int y = 1; y <= p.rozmiar_planszy; y++) {
			for (int x = 1; x <= p.rozmiar_planszy; x++) {
				if (p.mapa[y][x] == 'o' && p.mapa_atomow[y][x] == '*') {
					p.mapa[y][x] = 'O';
					liczba_poprawnie_zaznaczonych_atomow++;
				}
		...
		cout << "Liczba poprawnie zaznaczonych atomow: " << liczba_poprawnie_zaznaczonych_atomow << endl;
		cout << "Wcisnij enter, aby zobaczyc, gdzie byly ukryte.";
...
void zwolnij_plansze(Plansza& p) {
	...
	for (int i = 0; i < rozmiar; i++) {
		delete[] p.mapa[i];
		delete[] p.mapa_atomow[i];
	}
	delete[] p.mapa;
	delete[] p.mapa_atomow;
}

int main()
{
	Stos stos;
	int maksymalna_liczba_ruchow = 100;
	stos.ruchy_gracza = new Ruch[maksymalna_liczba_ruchow];
	...
	while (true) {
		...
		wyswietl_menu();
		poziom_gry(wybrana_plansza);
		alokuj_plansze(wybrana_plansza);
		generuj_plansze(wybrana_plansza);

		while (true) {
			wypisz_plansze(wybrana_plansza, pomoc, xGracz, yGracz);
			sterowanie(xGracz, yGracz, wybrana_plansza.rozmiar_planszy + 1, wybrana_plansza.rozmiar_planszy + 1, pomoc, przerwanie_gry, koniec_gry, poddanie_sie, wybrana_plansza, stos);
			wyczysc_ekran();
			if (przerwanie_gry) {
		...
		zwolnij_plansze(wybrana_plansza);
